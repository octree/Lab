...
class Renderer: NSObject {
    ...
    init(metalView: MTKView) {
        ...
        let rotation = float4x4(rotation: [0, Float(70).degreesToRadians, 0])
        ...
        let projectMatrix = float4x4(projectionFov: Float(45).degreesToRadians,
                                     near: 0.1,
                                     far: 100,
                                     aspect: aspect)
        uniforms.projectionMatrix = projectMatrix
    }
}
...
extension Renderer: MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        timer += 0.05
        uniforms.viewMatrix = float4x4(translation: [0, 0, -3]).inverse * float4x4(rotationY: sin(timer))
